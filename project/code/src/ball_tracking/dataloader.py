...
def generate_gaussian_heatmap(h, w, ball_x, ball_y, visibility, variance): 
    ...
    gaussian = np.exp(-((x - ball_x)**2 + (y - ball_y)**2) / (2 * variance))
    
    # make the heatmap range from 0 to 255
    heatmap = (gaussian * 255).astype(np.float32)
...
class BallDataset(Dataset):
    def __init__(self, type="train", frame="last", train_coef=0.7, val_coef=0.15, root_dir="/scratch/users/andyjalloh/ball_tracking_kaggle_dataset/",
        ...
        NORMALIZATION_PRESETS = {
            ...
            # Option B: Symmetric (current)
            "symmetric": transforms.Normalize(
                mean=[0.5, 0.5, 0.5], 
                std=[0.5, 0.5, 0.5]
            ),
            # Option C: No normalization
    ...
    def __getitem__(self, index):
        ...
        return torch.cat(list_img, dim=0), heatmap, visibility